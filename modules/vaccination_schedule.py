                ...
                with closing(con.cursor()) as cursor:
                    ...
                    for plan in plans:
                        ...
                        for affiliate in affiliates:
                                age = self.calculate_age(datetime.fromtimestamp(affiliate[7]))
                                if plan[1] <= age and age <= plan[2]:
                                    ans = ans and self.create_vaccination_schedule(date_obj.timestamp(), affiliate[0], lots[0][0], plan[0])
